...
public class WalletUtils
{
		...
		for (final TransactionOutput output : tx.getOutputs())
		{
			try
			{
				if (!output.isMine(wallet))
				{
					...
					return script.getToAddress(WalletConstants.NETWORK_PARAMETERS, true);
				}
			}
	...
	{
		is.mark((int) WalletConstants.BACKUP_MAX_CHARS);
	...
	public static void writeKeys(final Writer out, final List<ECKey> keys) throws IOException
	{
		...
		{
			out.write(key.getPrivateKeyEncoded(WalletConstants.NETWORK_PARAMETERS).toBase58());
	...
	public static List<ECKey> readKeys(final BufferedReader in, final NetworkParameters expectedNetworkParameters) throws IOException
	{
		try
		{
			...
			while (true)
			{
				...
				if (charCount > WalletConstants.BACKUP_MAX_CHARS)
					throw new IOException("read more than the limit of " + WalletConstants.BACKUP_MAX_CHARS + " characters");
	...
	public static final FileFilter KEYS_FILE_FILTER = new FileFilter()
	{
		...
		public boolean accept(final File file)
		{
			...
			try
			{
				...
				WalletUtils.readKeys(reader, WalletConstants.NETWORK_PARAMETERS);
